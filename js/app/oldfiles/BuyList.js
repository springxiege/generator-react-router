            ...
            if(narr.length > 1){
                ...
                if(narr.length > 0){
                    ...
                    for (let i = 0; i < narr.length; i++) {
                        let item = narr[i];
                        if(field.map((val) => first[val] == item[val]).reduce((prev,next) => prev&& next)){
                            next = i;
                            break;
                        }
                    }
                    // 由于在安卓手机中不支持ES6中findIndex新特性，故换上面的通用处理方式
                    // let next = narr.findIndex((value,index)=>{
        ...
        return (
            <div className="main">
                ...
                <footer className="cart-footer buy-footer clearfix">
                    ...
                    <a onClick={e=>this.handleBilling(e)}>确认</a>
                </footer>
            </div>
        )
    }
